...
function App() {
  ...
  //pure js section -------------
  // can do any js you want - inclduing if statemtns
  const movie = {
  ...
  // html with only limited js -----------
  // assumed to be HTML; use {...} to denote js
  // can only do a ternary operator (not an if statement)
  ...
  return (
    <div className="App">
      ...
      {
        movies.map( movie => {
          return <Movie data={movie} extra={ {a:1, b:2} } />
        })
      }      
    </div>
  );
